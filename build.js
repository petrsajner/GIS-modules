 ...
 *   node build.js --dev       → Dev server: http://localhost:7800/ (modules served separately)
 *
 ...
 *            writes dev/index.html, starts a mini HTTP server on port 7800, opens Chrome.
 */
...
// v203en: video.js rozdělený na 6 submodulů v přesném původním pořadí funkcí.
// Build order mezi nimi:
//   video-utils    → pure helpers (no deps on other video modules)
//   video-models   → VIDEO_MODELS, handlery, model UI switching, prompt rewriting
//   video-queue    → videoJobs state, _saveVideoResult, runVideoJob (volá modely)
//   video-gallery  → gallery UI, refs, mention, lightbox, source slots
//   video-topaz    → Topaz + Magnific upscale (volá _saveVideoResult z queue)
//   video-archive  → export/import (volá renderVideoGallery z gallery)
const MODULES = [
  'models.js',
  'styles.js',
  'setup.js',
...
function buildProd() {
  ...
  let html = template.replace(JS_MARKER, js);
  // Inject version badge — display as "v205" (strip "en" suffix for UI cleanliness)
  const versionDisplay = 'v' + String(VERSION).replace(/en$/i, '');
  html = html.replace(/__GIS_VERSION__/g, versionDisplay);
...
function generateDevHtml(template) {
  ...
  return template
    .replace(blockRegex, scriptTags)
    .replace(/__GIS_VERSION__/g, 'DEV');
}
